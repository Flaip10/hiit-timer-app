...
const bundleId =
  variant === 'development' ? `${baseId}.dev`
  : variant === 'preview'   ? `${baseId}.preview`
  : baseId;

const name =
  variant === 'development' ? 'Arc Timer (Dev)'
  : variant === 'preview'   ? 'Arc Timer (Preview)'
  : 'Arc Timer';

const scheme =
  variant === 'development' ? 'arctimer-dev'
  : variant === 'preview'   ? 'arctimer-preview'
  : 'arctimer';
...
const config: Config = {
  ...
  android: {
    ...
    versionCode: 2,
    adaptiveIcon: {
      foregroundImage: './assets/generated/classic/adaptive-foreground.png',
      monochromeImage: './assets/generated/classic/adaptive-monochrome.png',
